...
class WikiPagePatchTest < ActiveSupport::TestCase
  ...
  def test_search_by_anonymous
    User.current = nil
    result = WikiPage.search(@tokens).first
    assert result.include?(@page_with_matching_title)
    assert result.include?(@page_with_matching_text)
  end
  ...
  def test_search_by_title_only
    result = WikiPage.search(@tokens, nil, titles_only: true).first
    assert result.include?(@page_with_matching_title)
    assert !result.include?(@page_with_matching_text)
  end

  def test_search_by_all_words
    result = WikiPage.search(@tokens, nil, all_words: true).first
    assert !result.include?(@page_with_matching_title)
    assert !result.include?(@page_with_matching_text)
    assert result.include?(@page_with_matching_title_all_words)
    assert result.include?(@page_with_matching_text_all_words)
  end
